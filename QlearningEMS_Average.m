...
function QlearningEMS_Average(simParam,parentFolder)
...
maxRewardTheo = simParam.weightSOC + simParam.weightP_FC + simParam.weightP_batt;
...
% Variables to be modified during learning:
sum_reward = 0;
sum_iterations = 1; 
greedy = false; % false: exploration (random), true: exploitation
average_reward = 0;
...
% % % %         try % If error, restart the episode
            ...
            for h = 1:maxit
                ...
                if systemOn                 
                    alpha = min(0.99,log(sum_iterations)./(0.05*sum_iterations));
                    beta = 500 ./ (501 + sum_iterations);
                    % Update Q
                    Q(sIdx,aIdx_fc) = Q(sIdx,aIdx_fc) + alpha * ( reward - average_reward + discount*max(Q(snewIdx,:)) - Q(sIdx,aIdx_fc) ); % The line that makes everything !!!
                    
                    if greedy
                        % Average reward update
                        sum_reward = sum_reward + reward;
                        sum_iterations = sum_iterations + 1;
                        average_reward = (1-beta)*average_reward + beta*(sum_reward/sum_iterations);
                    end % end if greedy
...
% % % %             completed = 1;
            ...
            if ~lowSOC
                ...
                fprintf(resultsReport,'Average reward %3.2f/%3.2f \r\n',average_reward,maxRewardTheo);
                fprintf(resultsReport,'_______________\r\n\r\n');
